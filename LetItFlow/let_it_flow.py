...
class SC2_KNN_cluster_aware(torch.nn.Module):
    ...
    def __init__(self, pc1, K=16, d_thre=0.03):
        ...
        self.d_thre = d_thre
        self.K = K
        _, self.kNN, _ = knn_points(
            pc1, pc1, lengths1=None, lengths2=None, K=K, return_nn=True
        )
    ...
    def forward(self, flow, ids=None):
        ...
        target_dist = (
            target_keypts[:, :, None, :] - target_keypts[:, None, :, :]
        ).norm(dim=-1)
...
def center_rigidity_loss(pc1, flow, cluster_ids):
    ...
    pt_dist_to_center = pc1 - pts_centers[0, cluster_ids[0]].unsqueeze(
        0
    )  # .norm(dim=-1, p=1)
    flow_dist_to_center = (pc1 + flow) - flow_centers[0, cluster_ids[0]].unsqueeze(
        0
    )  # .norm(dim=-1, p=1)
