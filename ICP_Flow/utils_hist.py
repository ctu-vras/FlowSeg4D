...
def topk_nms(x, k=5, kernel_size=11):
    b, _, _, _ = x.shape
    x = x.unsqueeze(1)
    xp = torch.nn.functional.max_pool3d(
        x, kernel_size=kernel_size, stride=1, padding=(kernel_size - 1) // 2
    )
...
def estimate_init_pose_batch(config, src, dst):
    ...
    t_dynamic = (
        torch.stack(
            [
                bins_x[t_argmaxs // d // w % h],
                bins_y[t_argmaxs // d % w],
                bins_z[t_argmaxs % d],
            ],
            dim=-1,
        )
    ...
    _, errors = nearest_neighbor_batch(
        pcd1_.reshape(b * k, n, 3), pcd2_.reshape(b * k, n, 3)
    )
    _, errors_inv = nearest_neighbor_batch(
        pcd2_.reshape(b * k, n, 3), pcd1_.reshape(b * k, n, 3)
    )

    # using errors
    errors = (errors.view(b, k, n) * mask1[:, None, :]).sum(dim=-1) / mask1[
        :, None, :
    ].sum(dim=-1)
    errors_inv = (errors_inv.view(b, k, n) * mask2[:, None, :]).sum(dim=-1) / mask2[
        :, None, :
    ].sum(dim=-1)
