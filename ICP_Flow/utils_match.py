...
from ICP_Flow.utils_helper import (
    transform_points_batch,
    nearest_neighbor_batch,
    setdiff1d,
    match_segments_descend,
    pad_segment,
)
...
def match_pcds(config, src_points, dst_points, src_labels, dst_labels):
    ...
    labels_unq = torch.unique(
        torch.cat([src_labels_unq, dst_labels_unq], axis=0), return_counts=False
    )
    ...
    pairs = pairs[mask]
    pairs_true = sanity_check(
        config, src_points, dst_points, src_labels, dst_labels, pairs
    )

    if len(pairs_true) > 0:
        pairs_sta, transformations_sta = match_pairs(
            config, src_points, dst_points, src_labels, dst_labels, pairs_true
        )
    else:
        pairs_sta, transformations_sta = (
            torch.tensor([]).reshape(0, 10),
            torch.tensor([]).reshape(0, 4, 4),
        )
    ...
    if len(pairs_sta) < len(labels_unq):
        ...
        pairs = torch.stack(
            [
                src_labels_unq.repeat_interleave(len(dst_labels_unq)),
                dst_labels_unq.repeat(len(src_labels_unq)),
            ],
            dim=1,
        )
        pairs_true = sanity_check(
    ...
    if len(pairs_true) > 0:
        pairs_dyn, transformations_dyn = match_pairs(
            config, src_points, dst_points, src_labels, dst_labels, pairs_true
        )
    else:
        pairs_dyn, transformations_dyn = (
            torch.tensor([]).reshape(0, 10),
            torch.tensor([]).reshape(0, 4, 4),
        )

    pairs_matched = torch.cat([pairs_sta, pairs_dyn], dim=0)
    transformations_matched = torch.cat(
        [transformations_sta, transformations_dyn], dim=0
    )
...
def match_pairs(config, src_points, dst_points, src_labels, dst_labels, pairs):
    ...
    matrix_transformations = torch.zeros(
        (len(src_labels_unq), len(dst_labels_unq), 4, 4)
    )
    ...
    transformations = hist_icp(config, segs_src, segs_dst)
    errors, inliers, ratios, ious, translations, rotations = match_eval(
        config, segs_src, segs_dst, transformations
    )
    ...
    for k, (
        ...
        if not check_transformation(config, translation, rotation, min(iou)):
            continue
    ...
    if num_matches > 0:
        ...
        valid = matrix_errors_min[src_idxs, dst_idxs] < config["thres_error"]
        src_idxs = src_idxs[valid]
        dst_idxs = dst_idxs[valid]

        pairs = torch.cat(
            [
                src_labels_unq[src_idxs][:, None],
                dst_labels_unq[dst_idxs][:, None],
        ...
        pairs = torch.tensor([]).reshape(
            0,
            2
            + matrix_errors.shape[-1]
            + matrix_inliers.shape[-1]
            + matrix_ratios.shape[-1]
            + matrix_ious.shape[-1],
        )
...
def hist_icp(config, src, dst):
    ...
    with torch.no_grad():
        init_poses_ = estimate_init_pose(config, src_, dst_)
        transformations_ = apply_icp(config, src_, dst_, init_poses_)
...
def match_eval(config, pcd1, pcd2, transformations):
    ...
    src_inlier = torch.logical_and(src_error < config["thres_dist"], src_mask).float()
    dst_inlier = torch.logical_and(dst_error < config["thres_dist"], dst_mask).float()
    ...
    src_iou = torch.sum(src_inlier, dim=1) / (
        torch.sum(src_mask, dim=1)
        + torch.sum(dst_mask, dim=1)
        - torch.sum(dst_inlier, dim=1)
    )
    dst_iou = torch.sum(dst_inlier, dim=1) / (
        torch.sum(src_mask, dim=1)
        + torch.sum(dst_mask, dim=1)
        - torch.sum(src_inlier, dim=1)
    )

    src_error = (src_error * src_mask).sum(1) / src_mask.sum(1)
    dst_error = (dst_error * dst_mask).sum(1) / dst_mask.sum(1)

    src_mean = (src[:, :, 0:3] * src_mask[:, :, None]).sum(dim=1) / src_mask.sum(
        dim=1, keepdim=True
    )
    src_ori_mean = (pcd1[:, :, 0:3] * src_mask[:, :, None]).sum(dim=1) / src_mask.sum(
        dim=1, keepdim=True
    )

    translations = src_mean - src_ori_mean
    rotations = (
        pytorch3d_t.matrix_to_euler_angles(
            transformations[:, 0:3, 0:3], convention="ZYX"
        )
        * 180.0
        / np.pi
    )
    ...
    return (
        torch.stack([src_error, dst_error], dim=1),
